...
POSTS_LIMIT_PER_GROUP = int(os.getenv("POSTS_LIMIT_PER_GROUP", "150"))
POSTS_DAYS_BACK = int(os.getenv("POSTS_DAYS_BACK", "3"))
...
def main():
    ...
    # Чтобы понять, что именно не матчится — покажем примеры
    unmatched_examples = []
    ...
    for p in all_posts:
        ...
        post_url = normalize_url(
            get_first(p, ["post_url", "url"])
        )
        comment_count = get_first(p, [
            "num_comments",     
            "comment_count",
            "comments_count"
        ])
        ...
        post_id = get_first(p, ["post_id", "id"]) or make_post_id_fallback(gid, user_id, text, created_at, post_url)
    ...
    if unmatched_examples:
        print("\n⚠️ Examples of URL mismatch (raw -> normalized):")
        for raw_u, norm_u in unmatched_examples:
            print(f"   RAW : {raw_u}")
            print(f"   NORM: {norm_u}\n")

    if not post_rows:
        print("❌ Prepared 0 posts → nothing to write. (Almost always URL matching issue)")
        conn.close()
        return

    try:
        conn.execute("BEGIN")
        users_written = upsert_many(
            cur,
            "User",
            "user_id",
    ...
    for gid, cnt in sorted(group_counter.items(), key=lambda x: x[0]):
    ...
    if posts_written > 0:
        try:
            print("\n🚀 START COMMENTS SCRAPER (chained)")
