...
BATCH_SIZE = int(os.getenv("COMMENTS_BATCH_SIZE", "150"))
REQUEST_TIMEOUT = int(os.getenv("BD_TIMEOUT", "1200"))
PROGRESS_POLL_SEC = int(os.getenv("BD_PROGRESS_POLL", "10"))
MAX_WAIT_SEC = int(os.getenv("BD_MAX_WAIT", "1800"))
...
# Data sources
# -------------------------
def get_posts_for_comments(conn: sqlite3.Connection) -> List[Tuple[int, str, str]]:
    """
    Returns: [(post_row_id(INTEGER), post_id(TEXT), post_url(TEXT)), ...]
    Требуется, чтобы Post.id существовал и был заполнен.
    """
    cur = conn.cursor()
    rows = cur.execute(
        """
    ...
    for r in rows:
        if r["id"] is None:
            # важно: дальше будет FK на Post.id
            continue
        out.append((int(r["id"]), str(r["post_id"]), str(r["post_url"])))
    return out
...
def main():
    ...
    posts = get_posts_for_comments(conn)
    ...
    for batch in chunked(posts, BATCH_SIZE):
        ...
        try:
            ...
            for c in comments:
                ...
                # Extract user identifiers safely
                numeric_user_id = looks_like_numeric_user_id(
                    get_first(c, ["user_id", "profile_id", "fb_user_id"])
                )
                # token like pfbid... (keep only as a fallback string)
                raw_user_token = get_first(c, ["commentator_id", "commentator_profile_id", "commentator_profile"])

                user_name = get_first(c, ["user_name", "commentator_name", "name"])
                user_url = get_first(c, ["commentator_profile_url", "commentator_profile", "user_url", "profile_url"])

                # Upsert user only if we have a numeric user_id (stable)
                ...
                if numeric_user_id:
                    cur.execute(
                        ...
                        """,
                        (numeric_user_id, user_name, user_url),
                    )
                    if user_has_int_id:
                        r = cur.execute("SELECT id FROM User WHERE user_id=?", (numeric_user_id,)).fetchone()
                        user_rowid = int(r["id"]) if r and r["id"] is not None else None

                # Decide what to store into Comment.user_ref_id
                if "user_ref_id" in comment_cols:
                    if comment_user_ref_is_int:
                        # INTEGER FK -> User.id
                        # if we can't resolve user_rowid, write NULL (avoid FK fail)
                        user_ref_value = user_rowid
                        if user_ref_value is None and numeric_user_id:
                            skipped_fk_user += 1
                    else:
                        # TEXT -> store numeric user_id if we have it; else store token (pfbid...) if present; else NULL
                        user_ref_value = numeric_user_id or (str(raw_user_token) if raw_user_token else None)
                else:
                    user_ref_value = None  # no such column in schema
    ...
    if "user_ref_id" in comment_cols and comment_user_ref_is_int:
        print("Skip user FK (no User.id resolved):", skipped_fk_user)
